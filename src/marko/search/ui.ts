...
function Footer() {
  return h.footer("gh-w").props({ id: "glbfooter", role: "contentinfo" }).children(
    h.div().children(
      h.div().props({ id: "rtm_html_1650" }).children(
        h.div().props({ id: "rtm_html_1651" }),
        h.h2("gh-ar-hdn").children("Additional site navigation"),
        h.div("gffoot").props({ id: "gf-BIG" }).children(
                    ...
                    h.li("gf-li").children(
                      h.div("gf-flags-wpr").children(
                        h.a("thrd").props({
                          ...
                          h.b("gf-if gspr flus"),
                          " ",
                          h.b("gh-sprRetina").props({ id: "gf-fbtn-arr" }),
                        ),
                        h.div().props({ id: "gf-f" }).children(
                          h.ul("gf-ful").props({ role: "navigation" }).children(
        ...
        h.div().props({ id: "gf-t-box" }).children(
          h.table("gf-t").children(
            h.tbody().children(
              h.tr().children(
                h.td().props({ colSpan: 2 }).children(
                  h.ul("gf-lb").props({ id: "gf-l" }).children(
                    h.li("gf-li").children(
                      h.a("thrd gf-bar-a").props({
                        href: "http://www.ebayinc.com",
                        _exsp: "m571.l2602",
                      }).children("About eBay"),
                    ),
              ...
              h.tr().props({ valign: "top" }).children(
    ...
    return h.div("search-results-item").style(this.purchased === true ? PurchasedStyle : null).children(
      ...
      h.div("lvpic pic img left").children(
        h.div("lvpicinner full-width picW").children(
          h.a("img imgWr2").props({ href: `/buy/${item.id}` }).children(
            h.img().props({ src: item.image, alt: item.title }),
          ),
        ),
      ),
      ...
      this.purchased === true ?
        ...
        h.button("buy-now").props(BuyNowProps).events(this.handleBuyButtonClick).children("Buy now!"),
    );
  }
}
